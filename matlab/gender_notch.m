...
%filename='experiment_5_sample/emodb2.csv';
filename='experiment_5_sample/gender_running.csv';
...
[numbers, strings, raw] = xlsread('acctest4.xls');
global nextRow;
nextRow = size(raw, 1);
...
% Identify the frequency of the walking artifact
artifact_freq = 0.35; % Adjust based on analysis of the frequency response graph

% Design a notch filter to remove the walking artifact frequency
FsB = 1/mean(diff(numPhone(:,1))); % Sampling frequency 
wo = artifact_freq / (FsB / 2); % Normalized frequency
bw = wo / 35; % Bandwidth (adjust as needed)
disp(bw);

% Design the notch filter using iirnotch
[d, ~] = iirnotch(wo, bw);

% Apply the notch filter to the signal
filtered_signal = filtfilt(d, 1, signal);

% Step 2: Perform Adaptive Noise Cancellation using RLS Filtering
filter_order = 32; % Filter order
forgetting_factor = 0.91; % Forgetting factor for RLS filter
...
cleaned_signal_rls = zeros(size(filtered_signal)); % Output signal

% Perform RLS adaptive filtering
for n = filter_order:length(filtered_signal)
    input_vector = filtered_signal(n:-1:n-filter_order+1); % Input vector (reversed)
